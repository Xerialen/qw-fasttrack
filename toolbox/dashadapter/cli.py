...
from .render import default_map_dir, default_map_template, render_html
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--map-template", type=Path, default=None)
    return p
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not map_dir.is_dir():
        map_dir = None
    proposal = convert_path(a.obducera, map_dir)
    a.out_snapshots.write_text(dumps(proposal), encoding="utf-8")
    if a.out_html is not None:
        tpl_p = a.map_template or default_map_template()
        if not tpl_p.is_file():
            raise SystemExit(f"map-template saknas: {tpl_p}")
        html = render_html(proposal, tpl_p.read_text(encoding="utf-8"))
