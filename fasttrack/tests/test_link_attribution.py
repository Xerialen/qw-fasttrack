...
def _default_graph_path() -> Path:
    """Graf för SNG/proxy-testerna (ej spår A:s kärnsvit).

    Överstyr med FASTTRACK_GRAPH; faller annars tillbaka på det pinnade
    fasttrack-graph.json (Windows-maskinen) som sng-fixturen är inspelad mot.
    """
    if override := os.environ.get("FASTTRACK_GRAPH"):
        return Path(override)
    return Path("/mnt/c/Users/benya/projects/quakeworld/route-lab/qw-nav-viewer/overlays/fasttrack-graph.json")


GRAPH = _default_graph_path()
